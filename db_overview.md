...
| ------------- | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| `branches`    | Quản lý chi nhánh/cửa hàng. Dùng làm điểm neo cho POS, đơn hàng, ca bán, kho và quỹ. | Tên, địa chỉ, trạng thái.                                                                       | 1-n `pos_devices`, `orders`, `shifts`; được tham chiếu bởi nhiều bảng nghiệp vụ. |
...
| `products`   | Quản lý mặt hàng bán/tồn kho.                       | SKU, tên, mô tả, ảnh, giá bán, giá vốn, số lượng, đơn vị tính, trạng thái active. | Thuộc `categories`; được dùng trong `order_items`, `cart_items`, `stock_items`, `stock_receipt_detail`, `stock_take_items`. |
...
| `wallets`             | Quản lý số dư ví của customer.                         | Customer, balance, status.                                                                | 1-1 `customers`; có `wallet_transactions`.                             |
...
| Table              | Tính năng                                              | Dữ liệu chính                                                            | Liên kết                                                                                 |
| ------------------ | ------------------------------------------------------ | ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------- |
| `customers`        | Quản lý khách hàng, học sinh, giáo viên hoặc khách lẻ. | Mã customer, họ tên, phone, type, status, spending limit, user liên kết. | Có `orders`, `wallet`, `student_profile`, `student_classes`, `coupons`, `notifications`. |
