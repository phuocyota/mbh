...
| `product_price_history` | Lưu lịch sử thay đổi giá bán/giá vốn của sản phẩm, gồm snapshot ban đầu từ `products`. | Product, giá cũ/mới, giá vốn cũ/mới, loại thay đổi, ghi chú. | Thuộc `products`. |
...
| `fund_transactions`     | Nhật ký biến động quỹ và số dư sau giao dịch.                                            | Fund, type, amount, balance after, ref type/id, order id, note.                                                            | Thuộc `funds`; liên kết logic với các chứng từ qua `ref_type`, `ref_id`, `order_id`.                    |
...
| `money_vouchers`        | Chứng từ tiền tổng quát cho thu/chi liên quan quỹ, đơn hàng, nhà cung cấp hoặc ref khác. | Code, type, fund, amount, order, supplier, purpose, ref type/id, note.                                                     | Thuộc `funds`; tùy chọn tham chiếu `orders`, `suppliers`.                                               |
